...
results_save <- list()
meanVal = 0.5
results_simplify <- data.frame('length'=ns,
...
for(l in 1:length(ns)){
  ...
  for(i in 1:trials){
    set.seed(123456 * (l+1) + i + 1 + 123456789)
    cat(paste0(i,', '))
    ...
    # gSeg_MDT1 <- gseg1_new(ns[l], MDT1, pval.perm = T, pval.appr = F)
  ...
  saveRDS(results_save,
          file =paste0(path,"\\results\\t1_length_full.rds") )
  saveRDS(results_simplify,
          file =paste0(path,"\\results\\t1_length_simple.rds") )
}
...
############################
results_simplify <- readRDS(file =paste0(path,"\\results\\t1_length_simple.rds"))
library(tidyverse)

plot_15tree_len_nomean <- function(results_simplify){
  ...
  ggplot( data_plot ) +
    ...
    scale_linetype_manual(
      values=c('twodash','dashed','longdash','dotdash','dotdash'),
...
# Plot
tmp <- plot_15tree_len_nomean(results_simplify)


png(paste0(path,"\\figures\\len_power_t1.png"),width = 1600,height = 800)
print(tmp)
dev.off()

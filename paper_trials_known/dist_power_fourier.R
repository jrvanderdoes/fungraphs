...
for(l in 1:length(dists)){
  ...
  for(i in 1:trials){
    ...
    data[[i]] <- gen_FD_KL_Expansion(
      ...
      eigsList = list(c(3,2,1,0.5),
                      c(3,2,1,0.5)),
      basesList = list(create.fourier.basis(nbasis=4),
                       create.fourier.basis(nbasis=4)),
      meansList = c(0,0),
  ...
  saveRDS(data,
          file=paste0(path,"\\results\\distribution_change_data_",l,"_new.rds") )
  saveRDS(results_save,
          file=paste0(path,"\\results\\distribution_change_full_new.rds") )
  saveRDS(results_simplify,
          file=paste0(path,"\\results\\distribution_change_simple_new.rds") )
}

for(l in 1:length(dists)){
  results_simplify$ce[l] <-
    readRDS(paste0(path,"\\results\\distribution_change_simple_ce",l,"_new.rds") )$ce[l]
}
saveRDS(results_simplify,
        file=paste0(path,"\\results\\distribution_change_simple_new_comb.rds") )
...
plot_15tree_dist <- function(results_simplify){
  ...
  ggplot( data_plot ) +
    geom_hline(aes(yintercept=0.05), color='gray',linetype='solid',linewidth=1.75) +
...
png(paste0(path,"\\figures\\dist_power_fourier.png") ,

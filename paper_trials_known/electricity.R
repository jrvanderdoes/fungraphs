...
errors <- readRDS(paste0(path,'/data/elec_errs.rds'))
electricity <- readRDS(paste0(path,'/data/electricity.rds'))
...
tmp <- plot_dates(electricity)
plotly::save_image(tmp,
                   file=paste0(path,"/figures/electricity.png"))
tmp <- plot_dates(errors)
plotly::save_image(tmp,
                   file=paste0(path,"/figures/electricity_errors.png"))
...
set.seed(12345)
res <- bs_func(errors,graph_change)
res1 <- ver_func(errors,res)
...
plot_cps(data = errors,CPs = res1)

tmp <- plot_cps(data = errors,CPs = res1)
plotly::save_image(tmp,
                   file=paste0(path,"/figures/electricity_errors_segments.png"))
colnames(errors)[res1]

...
for(l in 1:length(deps)){
  ...
  results_simplify[l,] <-
    c(deps[l], as.numeric(colSums(results<=0.05,na.rm = T)/colSums(!is.na(results))))

  saveRDS(results_save,
          file=paste0(path,"\\results\\dep_change_full.rds") )
  saveRDS(results_simplify,
          file=paste0(path,"\\results\\dep_change_simple.rds") )
}


######################################
results_simplify <- readRDS(paste0(path,'\\results\\dep_change_simple.rds'))
...
tmp <- plot_15tree_dep(results_simplify)

png(paste0(path,"\\figures\\dep_power_change.png"),

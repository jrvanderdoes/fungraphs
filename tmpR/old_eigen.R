...
results_simplify_L2 <- results_simplify_CE <-
...
for(l in 1:length(eigs)){
  ...
  for(i in 1:trials){
    ...
    data_CE[[i]] <- calculateDistanceMatrix(data[[i]], silent = T,
                                            errType='L2', dataUse = 'CE')

    results_tmp <- data.frame('MDT1ori'=1:2, 'MDT1wei'=NA,
                              'MDT1max'=NA, 'MDT1gen'=NA,
                              'MDT3ori'=NA, 'MDT3wei'=NA,
                              'MDT3max'=NA, 'MDT3gen'=NA,
                              'MDT5ori'=NA, 'MDT5wei'=NA,
    ...
    for(dat_idx in 1:2){
      if(dat_idx == 1) dat <- data_L2[[i]]
      if(dat_idx == 2) dat <- data_CE[[i]]
    ...
    results_L2[i,] <- results_tmp[1,]
    results_CE[i,] <- results_tmp[2,]

    sink()

    if ((i %% 50) ==0){
      saveRDS(results_L2, paste0('C:/Users/jerem/OneDrive/Documents/',
                                 'School/Waterloo/Research/',
                                 'GraphCP/Data/Simulations/',
                                 'tmp_t',trials,'_l',l,'_eigs05_L2','.rds'))
      saveRDS(results_CE, paste0('C:/Users/jerem/OneDrive/Documents/',
                                 'School/Waterloo/Research/',
                                 'GraphCP/Data/Simulations/',
                                 'tmp_t',trials,'_l',l,'_eigs95_CE','.rds'))

    }
  }

  results_simplify_L2[l,] <-
    c(eigs[l], as.numeric(colSums(results_L2<=0.05,na.rm = T) /
                            colSums(!is.na(results_L2))))
  results_simplify_CE[l,] <-
    c(eigs[l], as.numeric(colSums(results_CE<=0.05,na.rm = T) /
                            colSums(!is.na(results_CE))))
  #
  ...
  #                              '_dep_05',
  #                              '_results.rds'))
  saveRDS(list(results_simplify_L2,results_simplify_CE),
...
# Plot
results_simplify_CE$mean <- 1-as.numeric(results_simplify_CE$mean)
results_simplify_L2$mean <- 1-as.numeric(results_simplify_L2$mean)
results_simplify_CE$cov <- 1-as.numeric(results_simplify_CE$cov)
results_simplify_L2$cov <- 1-as.numeric(results_simplify_L2$cov)

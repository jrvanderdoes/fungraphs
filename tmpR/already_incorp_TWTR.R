...
```{readData r}
TWTR1=readRDS('C:\\Users\\jerem\\Downloads\\TWTR.rds')
TWTR <- read.csv(paste0(data_path,'TWTR.txt'))#readRDS('C:\\Users\\j53vande\\Downloads\\TWTR.txt')
TWTR <- TWTR[,c("X.DATE.","X.TIME.","X.CLOSE.")]
colnames(TWTR) <- c('date','time','close')
TWTR$dateTime <- as.POSIXct.Date(paste0(TWTR$date, ' ', TWTR$time),'%Y%m%d "%h%M%S')

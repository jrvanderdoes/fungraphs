gseg1_new = function (n, E, statistics = c("all", "o", "w", "g", "m"), n0 = 0.05 * 
            n, n1 = 0.95 * n, pval.appr = TRUE, skew.corr = TRUE, pval.perm = FALSE, 
          B = 100) 
{
  r1 = list()
  n0 = ceiling(n0)
  ...
  if (skew.corr == FALSE) {
    if (length(which(!is.na(match(c("o", "ori", "original", 
      ...
      if (b > 0) {
        ...
        pval.ori = dnorm(b) * b * integrate(integrandO, 
    ...
    if (length(which(!is.na(match(c("w", "weighted", "all"), 
      ...
      if (b > 0) {
        ...
        pval.weighted = dnorm(b) * b * integrate(integrandW, 
                                                 lower, upper, subdivisions = 3000, stop.on.error = FALSE)$value
      }
      else {
        pval.weighted = 1
    ...
    if (length(which(!is.na(match(c("m", "max", "all"), 
      ...
      if (b > 0) {
        ...
        pval_u1 = 2 * dnorm(b) * b * integrate(integrand1, 
                                               lower, upper, subdivisions = 3000, stop.on.error = FALSE)$value
        pval_u2 = dnorm(b) * b * integrate(integrand2, 
                                           lower, upper, subdivisions = 3000, stop.on.error = FALSE)$value
    ...
    if (length(which(!is.na(match(c("g", "generalized", 
      ...
      if (b > 0) {
        ...
        integrand0 = function(t) {
          integrate(integrandG, 0, 2 * pi, t = t, subdivisions = 3000, 
                    stop.on.error = FALSE)$value
        }
        pval.generalized = dchisq(b, 2) * integrate(Vectorize(integrand0), 
                                                    lower, upper, subdivisions = 3000, stop.on.error = FALSE)$value
      }
      else {
        pval.generalized = 1
  ...
  if (length(which(!is.na(match(c("o", "ori", "original", 
    ...
    if (b > 0) {
      ...
      if (nn > 0.75 * n) {
        ...
        pval.ori = dnorm(b) * b * integrate(integrand, 
                                            lower, upper, subdivisions = 3000, stop.on.error = FALSE)$value
        output$ori = min(pval.ori, 1)
      }
      else {
        ...
        result = try(dnorm(b) * b * integrate(integrand, 
                                              lower, upper, subdivisions = 3000, stop.on.error = FALSE)$value, 
                     silent = T)
